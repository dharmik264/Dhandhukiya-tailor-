...
class DashboardActivity : AppCompatActivity() {
    ...
    private fun loadRecentOrders() {
        // Try backend first so data is always up to date
            ...
            override fun onResponse(
                ...
                if (response.isSuccessful) {
                    val list = (response.body() ?: emptyList()).map {
                        CheckOrderModel(it.id, it.customerName, it.garmentType)
                    }
                    recentOrdersAdapter.updateData(list)
                } else {
                    android.util.Log.w("DASHBOARD", "Backend returned ${response.code()}, falling back to local DB")
                    loadRecentOrdersFromLocal()
                }
            }

            override fun onFailure(call: retrofit2.Call<List<RecentOrderResponse>>, t: Throwable) {
                android.util.Log.w("DASHBOARD", "Network failure for recent orders, falling back to local DB", t)
                loadRecentOrdersFromLocal()
            }
        })
    }

    private fun loadRecentOrdersFromLocal() {
        thread {
            try {
                ...
                android.util.Log.e("DASHBOARD", "Failed to load recent orders from local DB", e)
            }
        }
    }
